...
def render_reward_text(window, cumulative_reward, gameboard_size):
    """
    Render the cumulative reward text on the screen
    
    Args:
        window: pygame display surface
        cumulative_reward: Current episode cumulative reward
        gameboard_size: Size of the gameboard
    """
    font = pygame.font.SysFont(None, 48)
    reward_text = f"Cumulative Reward: {cumulative_reward:.2f}"
...
def main():
    print("Starting MAISR Human Interface")
    ...
    while running:
        ...
        while not (terminated or truncated) and running:
            ...
            # Use current action or default to no movement
            if current_action is not None:
                action = current_action
            else:
                # Default action: stay in place (agent's current position normalized)
                agent_x = env.agents[env.aircraft_ids[0]].x
                agent_y = env.agents[env.aircraft_ids[0]].y
                action = convert_game_to_action_space((agent_x, agent_y), gameboard_size)
            ...
            # Render cumulative reward on top of the game
            render_reward_text(window, cumulative_reward, gameboard_size)

...
class MAISREnv(gym.Env):
    ...
    def __init__(self, config={}, window=None, clock=None, render_mode='none',
        ...
        if "seed" in config:
            ...
            print("Note: no 'seed' specified in the env config.")
            # random.seed(0)
    ...
    def step(self, actions:list):
        ...
        #     agent_action = (float(agent_action[0]), float(agent_action[1]))
        #     waypoint_queue.append((self.agent_idx, agent_action))
        # else:
        #     raise Exception(f'Agent action not found in actions list')
        #
        # if human_action is not None:
        #     human_action = (float(human_action[0]), float(human_action[1]))
        #     waypoint_queue.append((self.human_idx, human_action))
        #
        ...
        # TODO add if any actions in the queue have the same agent_id, print a statement and take the last action only
        ...
        for agent in self.agents:
            ...
            if agent.agent_class == "ship":
                for aircraft_id in self.aircraft_ids: # check if ship is within range of aircraft

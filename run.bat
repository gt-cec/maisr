...
set /p cond=Enter the study condition (1: control, 2: card, 3: in-situ, 4: test, 5: transparency):
set /p order=Enter the run order:
set round_num=0
...
if "%cond%"=="5" set cond="transparency"
...
echo Starting pygame with ID %userid%, condition %cond%, run order %order%, and round %round_num%
python main.py %userid% %cond% %order% %round_num% %log_data%
goto end

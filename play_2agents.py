...
from utility.league_management import (TeammateManager, RLTeammatePolicy)
...
def run_single_episode(env, human_controller, config, config_index, total_configs, agent_model, window, font, clock, tick_rate, data_logger, time_factor, teammate, mode):
    ...
    if mode != 'human':
        env.envs[0].env.current_teammate = teammate
    ...
    while not done:
        ...
        # Get human action
        human_action = human_controller.get_current_action() # TODO REMOVE
        if mode == 'human':
            ...
            pass # teammate action should be handled in the wrapper
    ...
    try:
        ...
        if args.mode == 'human':
            ...
            if args.mode == 'selfplay':
                teammate = RLTeammatePolicy(
                    model=PPO.load(f'experiments/exp2_user_study/saved_agents/{agent_a_name}_model.zip'),
                    env=None,
                    local_search_policy=None,
                    go_to_highvalue_policy=None,
                    change_region_subpolicy=None,#
                    norm_stats_path=f'experiments/exp2_user_study/saved_agents/{agent_a_name}_vecnormalize.pkl'
                )

            elif args.mode == 'heuristic':
                tm = TeammateManager()
                tm.overfit_test = 'low_risk'
                teammate = tm._create_overfit_test_teammate()

        should_quit, episode_reward, step_count = run_single_episode(
            env, human_controller, practice_cfg, 0, len(full_config_list),
            current_agent_name, window, font, clock, tick_rate, data_logger, time_factor, teammate, args.mode)
